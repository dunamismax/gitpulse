...
[![CI](https://github.com/dunamismax/gitpulse/actions/workflows/ci.yml/badge.svg)](https://github.com/dunamismax/gitpulse/actions/workflows/ci.yml) [![Go](https://img.shields.io/badge/Go-1.26.1-00ADD8.svg)](go.mod) [![License: GPL-3.0](https://img.shields.io/badge/license-GPL--3.0-blue.svg)](LICENSE)
...
> **Status:** Active and usable today as a Go CLI plus local web dashboard. `gitpulse serve` serves the built Astro + Vue frontend directly from Go while keeping the Go JSON API as the source of truth. GitPulse is manual-first today: add repositories, import history, rescan working trees, and rebuild analytics explicitly. `gitpulse tui` provides a keyboard-driven terminal preview of that same operator loop, including repo jump search and page navigation for larger inventories, but it remains a secondary source-run surface. Background watchers or pollers and packaged desktop releases are not shipped yet. The shipped implementation is still the current Go + SQLite system, while [BUILD.md](BUILD.md) now tracks the planned full rewrite to Stephen's unified Bun + TypeScript + Astro + Vue + Elysia + PostgreSQL + Docker Compose + Caddy stack. Rewrite-boundary docs that freeze the current contract and migration edge live in [docs/rewrite/current-contract.md](docs/rewrite/current-contract.md) and [docs/rewrite/vnext-boundary.md](docs/rewrite/vnext-boundary.md). The current operator flow lives in [docs/operator-workflow.md](docs/operator-workflow.md).
