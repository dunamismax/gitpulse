...
import type { DashboardView, SettingsView } from '@gitpulse-vnext/contracts';
import { computed, onMounted, ref } from 'vue';
import { GitPulseClient, GitPulseClientError } from '../lib/client';
import {
  formatDateTime,
  formatMinutes,
  heatmapClass,
      ...
      <section class="split-grid">
        <article class="panel">
          ...
          <div v-if="dashboard.activity_feed.length" class="list-stack">
            <div
              v-for="item in dashboard.activity_feed"
              :key="`${item.kind}:${item.timestamp}:${item.repo_name}`"
              class="list-row"
            >
              <div>
      ...
      <section class="panel">
        <div class="panel-head">
          <div>
            ...
            <p class="panel-copy">
              Quick status for tracked repositories on the PostgreSQL-backed
              vNext runtime.
            </p>
          </div>
        ...
        <div v-if="dashboard.repo_cards.length" class="repo-grid">
          <article
            v-for="card in dashboard.repo_cards"
            :key="card.repo.id"
